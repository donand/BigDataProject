...
def cluster_by_time(start_time, end_time, eps, min_samples, filename):
    mask = (df_position['date'] > pd.to_datetime('11-04-2016',format='%d-%m-%Y')) & (df_position['date'] <= pd.to_datetime('18-04-2016',format='%d-%m-%Y')) & (df_position.date.dt.hour < end_time) & (df_position.date.dt.hour >= start_time) & (df_position.latitude > 45.443041) & (df_position.longitude > 9.133225) & (df_position.latitude < 45.500763) & (df_position.longitude < 9.252532)
